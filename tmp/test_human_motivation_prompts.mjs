...
assert.match(systemPrompt, /写了动作或反应后/)
assert.match(systemPrompt, /不要马上翻译成情绪结论/)
assert.match(systemPrompt, /不能主动长篇交底/)
assert.match(systemPrompt, /不要写成干净的计划书/)
assert.doesNotMatch(systemPrompt, /整章最多 2 次/)
...
assert.match(chapterPrompt, /有效选择/)
assert.match(chapterPrompt, /不同选择/)
assert.match(chapterPrompt, /信息揭示方式/)
assert.match(chapterPrompt, /证据、动作失败、物件反应和人物选择/)
assert.doesNotMatch(chapterPrompt, /整章最多 2 次/)
...
assert.match(auditPrompt, /综合判断/)
assert.doesNotMatch(auditPrompt, /超过 2 次/)
