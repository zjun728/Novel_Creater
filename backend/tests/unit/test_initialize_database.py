...
async def test_fresh_database_executes_manifest_in_order_and_writes_metadata():
    ...
    assert EXPECTED_SCHEMA_VERSION == "writer-core-v1.12.0"
    ...
    assert executed == [
        ...
        *((statement, None) for statement in read_statements()),
        (
            metadata_insert,
        ("writer-core-v1.12.0", manifest_hash(), 1_720_000_000_123),
        ),
    ]
    ...
    assert result.table_count == len(created_table_names()) == 89
