...
@pytest.mark.asyncio
async def test_frozen_v11_rebuilds_to_v14_then_v14_execute_is_noop(
    ...
    assert dry_receipt["target"]["kind"] == "v1.4-target"
    ...
    assert noop_receipt["source"]["kind"] == "v1.4-target"
...
async def test_reset_classification_fails_closed_before_ddl(
    ...
    with pytest.raises(ResetValidationError, match="v1.1 source or v1.4 target"):
...
@pytest.mark.asyncio
async def test_v14_noop_rejects_archived_foundation_before_ddl(
    empty_disposable_mysql,
):
    await create_frozen_v11_product_state(empty_disposable_mysql)
    await reset_writer_core_data(
