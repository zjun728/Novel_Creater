...
<template>
  <section class="foundation-workspace" :class="{ 'foundation-workspace--readonly': readOnly }">
    <header class="foundation-workspace__header">
      ...
      <p v-if="statusLabel" class="foundation-workspace__status">{{ statusLabel }}</p>
    </header>
...
<style scoped>
.foundation-workspace { min-width:0; padding:clamp(22px,4vw,52px); overflow-wrap:anywhere; color:var(--nc-ink); background:var(--nc-canvas); }
...
.foundation-workspace__status { align-self:end; margin:0; color:var(--nc-vermilion); font:700 12px Georgia,'Noto Serif SC',serif; letter-spacing:.08em; text-align:right; }
.foundation-workspace__grid { display:grid; grid-template-columns:minmax(168px,.72fr) minmax(0,2.5fr) minmax(196px,.86fr); gap:18px; width:min(1320px,100%); min-width:0; margin:auto; align-items:start; }
.foundation-workspace__index,.foundation-workspace__document,.foundation-workspace__status { min-width:0; }
.foundation-workspace__index,.foundation-workspace__status { border-top:1px solid var(--nc-border); }
.foundation-workspace__document { min-height:clamp(420px,68vh,760px); border:1px solid var(--nc-border); background:repeating-linear-gradient(0deg,var(--nc-paper),var(--nc-paper) 29px,color-mix(in srgb,var(--nc-paper) 94%,var(--nc-canvas)) 30px); box-shadow:0 24px 64px color-mix(in srgb,var(--nc-ink) 9%,transparent); }
...
@media (max-width:760px) { .foundation-workspace { padding:18px 12px 30px; } .foundation-workspace__header { grid-template-columns:1fr; } .foundation-workspace__status { justify-self:start; text-align:left; } .foundation-workspace__grid { grid-template-columns:minmax(0,1fr); } .foundation-workspace__document { min-height:0; } }
