...
<style scoped>
.history-overlay { position:fixed; inset:0; z-index:20; display:flex; justify-content:flex-end; background:color-mix(in srgb,var(--nc-ink) 38%,transparent); }.history-sheet { width:min(460px,100%); overflow:auto; padding:28px; color:var(--nc-ink); background:var(--nc-paper); box-shadow:-16px 0 48px color-mix(in srgb,var(--nc-ink) 22%,transparent); }.history-note { color:var(--nc-muted); font-size:12px; line-height:1.65; }.history-sheet article { margin:14px 0; padding:14px; border:1px solid var(--nc-border); }.history-sheet button { margin:6px 0; color:var(--nc-vermilion); }.history-detail { min-width:0; color:var(--nc-muted); }.history-detail dd { min-width:0; overflow-wrap:anywhere; word-break:break-word; }.modal-error-summary { display:grid; gap:8px; margin:12px 0; padding:12px; border:1px solid var(--nc-vermilion); color:var(--nc-ink); background:color-mix(in srgb,var(--nc-paper) 92%,var(--nc-canvas)); }
</style>
