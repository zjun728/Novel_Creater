...
${seeds?.length ? `\n## 已有创作种子\n${seeds.map((s, i) => {
    const status = s.status === 'selected' ? '当前选中' : '候选'
    return `${i + 1}. [${status}] ${s.title}（${s.genre}）- ${s.logline}`
  }).join('\n')}` : ''}
...
- 当用户明确要求时才生成种子
- 当用户要求“修改当前种子/更新当前种子/更新最新种子/调整这个种子/把种子改成...”时，请基于当前选中的种子和最近对话，输出修改后的完整单个种子 JSON；系统会应用到当前选中的种子，不会另存为新候选
- 种子必须有差异化理由，不能是市场数据的简单复刻
- 结局锚点字段必须保留；它是长篇小说的终局方向，不要求剧透所有细节，但应尽量给出最终画面、情绪收束或主题归宿；如果用户信息确实不足，可以填空字符串，不要删除字段
