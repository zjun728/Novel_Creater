...
const bookSelector = computed(() => ({ scope: 'book', format: download.options.value?.formats?.includes('txt') ? 'txt' : download.options.value?.formats?.[0] }))
...
async function downloadBook() { if (bookSelector.value.format) { try { await download.download(projectId.value, bookSelector.value) } catch {} } }
async function downloadChapter(chapterNumber) {
  const format = bookSelector.value.format
...
<template>
  <main class="manuscript-index" :aria-busy="manuscript.content.value.status === 'loading'">
    ...
    <n-result v-else-if="manuscript.content.value.status === 'missing-project'" status="404" title="项目不存在或已被删除" description="请返回项目库确认项目状态。" />
    ...
    <section v-else class="manuscript-index__sheet" aria-labelledby="manuscript-index-title">
      ...
      <div v-if="hasChapters" class="manuscript-index__downloads">
        <button v-if="download.options.value?.available" type="button" @click="downloadBook">下载整本定稿</button>
        <p v-else-if="download.error.value" class="manuscript-index__local-error" role="alert">下载选项暂时无法加载。<button type="button" @click="loadOptions">重新读取</button></p>
      </div>

      <p v-if="manuscript.content.value.status === 'empty'" class="manuscript-index__empty">还没有已定稿章节</p>
      <manuscript-chapter-list v-else-if="directory" :project-id="projectId" :volumes="directory.volumes" :download-chapter="downloadChapter" :can-download-chapter="number => chapterOptions.has(number)" />
