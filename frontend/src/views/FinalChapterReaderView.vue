...
<template>
  <section class="final-reader" aria-labelledby="final-reader-title" :aria-busy="status === 'loading'">
    ...
    <header class="final-reader__header">
      ...
      <h1 id="final-reader-title">{{ data ? `第 ${data.chapter.number} 章 · ${data.chapter.title}` : '章节定稿' }}</h1>
      <router-link v-if="preparation.status === 'ready' && !isArchived && status !== 'missing-project'" class="final-reader__action" :to="preparation.nextAction.targetPath">{{ preparation.nextAction.label }}</router-link>
      <p v-else-if="preparation.status === 'loading' && chapterNumber" class="final-reader__preparation" role="status">正在读取当前创作位置</p>
      <p v-else-if="preparation.status === 'unavailable' && chapterNumber && status !== 'missing-project'" class="final-reader__local-error">创作状态暂时无法读取。<button type="button" @click="retryPreparation">重新读取创作状态</button></p>
    </header>
    ...
    <section v-else-if="data" class="final-reader__sheet">
      <p class="final-reader__meta">第{{ data.volume.order }}卷 · {{ data.volume.title }} · {{ data.chapter.scalarCount.toLocaleString('zh-CN') }} 字 · <time :datetime="data.chapter.finalizedAt">{{ new Date(data.chapter.finalizedAt).toLocaleDateString('zh-CN') }}</time></p>
...
.final-reader__header, .final-reader__sheet, .final-reader__notice { width: min(760px, 100%); margin: 0 auto; padding: clamp(24px, 4vw, 48px); border: 1px solid var(--nc-border); background: var(--nc-paper); box-shadow: 0 24px 64px rgba(58, 43, 27, .07); }
.final-reader__sheet, .final-reader__notice { margin-top: 16px; }
...
.final-reader__action { display: inline-flex; min-height: 44px; margin-top: 20px; padding: 0 16px; align-items: center; color: var(--nc-paper); background: var(--nc-ink); text-decoration: none; }
...
@media (max-width: 560px) { .final-reader__tabs, .final-reader__download > div { display: grid; } .final-reader__tabs button, .final-reader__download button { width: 100%; } }
</style>
