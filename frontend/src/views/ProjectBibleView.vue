...
async function clone(source) {
  try { await workspace.clone(source); historyOpen.value = false; notice.value = '已创建未来设计草稿' }
  catch (error) { notice.value = store.error?.message || error.message }
}
