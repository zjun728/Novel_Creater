...
test('writer finalization reloads server actions and verifies the committed chapter without navigating', async () => {
  const view = await source('views/ChapterWriterView.vue')
  const commit = functionBody(view, 'commit: async (command, target) =>')
  ...
  assert.match(view, /getSessionId:\s*\(\)\s*=>\s*session\.value\?\.id/)
  ...
  assert.match(commit, /chapterNumber:\s*target\.chapterNumber/)
  assert.match(view, /commit:\s*async\s*\(command, target\)\s*=>/)
  assert.match(commit, /target\.projectId/)
  assert.match(commit, /target\.sessionId/)
  assert.match(commit, /target\.chapterNumber/)
  assert.doesNotMatch(commit, /projectId\.value|session\.value|chapterNumber\.value/)
