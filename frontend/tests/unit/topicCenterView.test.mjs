...
test('market and discussion surfaces preserve explicit evidence and explicit saves', async () => {
  ...
  assert.match(presentation, /仅支持导入/)
  ...
  assert.match(market, /marketFailureCopy\(source, market\.sourceState\(source\.id\)\.snapshots\)/)
  assert.match(market, /marketSnapshotDisplayName\(snapshot, snapshot\.source\)/)
