...
function safeSpecProjectionLine(message) {
  ...
  const healthSummary = /^category=audit leaf=runtime-health-summary responseFailureCount=(\d+) consoleErrorCount=(\d+) pageErrorCount=(\d+) requestFailureCount=(\d+) apiReadErrorCount=(\d+) requestReadErrorCount=(\d+) forbiddenRequestCount=(\d+) forbiddenResponseCount=(\d+) responseMethod=(GET|POST|PUT|PATCH|DELETE|HEAD|OPTIONS|unavailable) responsePath=(\/api(?:\/[A-Za-z0-9._~:-]+)*|unavailable) responseStatus=([1-5]\d{2}|unavailable) requestMethod=(GET|POST|PUT|PATCH|DELETE|HEAD|OPTIONS|unavailable) requestPath=(\/api(?:\/[A-Za-z0-9._~:-]+)*|unavailable) requestStatus=unavailable readMethod=(GET|POST|PUT|PATCH|DELETE|HEAD|OPTIONS|unavailable) readPath=(\/api(?:\/[A-Za-z0-9._~:-]+)*|unavailable) readStatus=([1-5]\d{2}|unavailable) responseInventory=(none|(?:GET|POST|PUT|PATCH|DELETE|HEAD|OPTIONS):\/api(?:\/[A-Za-z0-9._~:-]+)*:[1-5]\d{2}:[1-9]\d*(?:\|(?:GET|POST|PUT|PATCH|DELETE|HEAD|OPTIONS):\/api(?:\/[A-Za-z0-9._~:-]+)*:[1-5]\d{2}:[1-9]\d*){0,7}) unavailableCount=(\d+) inventoryOmittedCount=(\d+) consoleExact404Count=(\d+) consoleExact409ConflictCount=(\d+) consoleFailedResource409OtherCount=(\d+) consoleVueCapturedErrorCount=(\d+) consoleCategoryOtherCount=(\d+) consoleCategoryOtherApiErrorCount=(\d+) consoleCategoryOtherErrorCount=(\d+) consoleCategoryOtherVueWarnCount=(\d+) consoleCategoryOtherUncaughtCount=(\d+) consoleCategoryOtherAccessFetchCount=(\d+) consoleCategoryOtherFailedResourceCount=(\d+) consoleCategoryOtherGenericErrorCount=(\d+) consoleCategoryOtherNonErrorPrefixCount=(\d+) consoleCategoryOther409TokenCount=(\d+) consoleKnownLinkedCount=(\d+) consoleOtherCount=(\d+)$/u.exec(message)
  if (healthSummary) {
    const [, responseFailureCount, consoleErrorCount, pageErrorCount, requestFailureCount, apiReadErrorCount, requestReadErrorCount, forbiddenRequestCount, forbiddenResponseCount, responseMethod, responsePath, responseStatus, requestMethod, requestPath, readMethod, readPath, readStatus, responseInventory, unavailableCount, inventoryOmittedCount, consoleExact404Count, consoleExact409ConflictCount, consoleFailedResource409OtherCount, consoleVueCapturedErrorCount, consoleCategoryOtherCount, consoleCategoryOtherApiErrorCount, consoleCategoryOtherErrorCount, consoleCategoryOtherVueWarnCount, consoleCategoryOtherUncaughtCount, consoleCategoryOtherAccessFetchCount, consoleCategoryOtherFailedResourceCount, consoleCategoryOtherGenericErrorCount, consoleCategoryOtherNonErrorPrefixCount, consoleCategoryOther409TokenCount, consoleKnownLinkedCount, consoleOtherCount] = healthSummary
    if (
    ...
    if (
      Number(consoleExact404Count)
      + Number(consoleExact409ConflictCount)
      + Number(consoleFailedResource409OtherCount)
      + Number(consoleVueCapturedErrorCount)
      + Number(consoleCategoryOtherCount)
      !== Number(consoleErrorCount)
    ) return null
    if (
      Number(consoleCategoryOtherApiErrorCount)
      + Number(consoleCategoryOtherErrorCount)
      + Number(consoleCategoryOtherVueWarnCount)
      + Number(consoleCategoryOtherUncaughtCount)
      + Number(consoleCategoryOtherAccessFetchCount)
      + Number(consoleCategoryOtherFailedResourceCount)
      + Number(consoleCategoryOtherGenericErrorCount)
      + Number(consoleCategoryOtherNonErrorPrefixCount)
      !== Number(consoleCategoryOtherCount)
      || Number(consoleCategoryOther409TokenCount) > Number(consoleCategoryOtherCount)
    ) return null
    return `category=audit leaf=runtime-health-summary responseFailureCount=${responseFailureCount} consoleErrorCount=${consoleErrorCount} pageErrorCount=${pageErrorCount} requestFailureCount=${requestFailureCount} apiReadErrorCount=${apiReadErrorCount} requestReadErrorCount=${requestReadErrorCount} forbiddenRequestCount=${forbiddenRequestCount} forbiddenResponseCount=${forbiddenResponseCount} responseMethod=${responseMethod} responsePath=${responsePath} responseStatus=${responseStatus} requestMethod=${requestMethod} requestPath=${requestPath} requestStatus=unavailable readMethod=${readMethod} readPath=${readPath} readStatus=${readStatus} responseInventory=${responseInventory} unavailableCount=${unavailableCount} inventoryOmittedCount=${inventoryOmittedCount} consoleExact404Count=${consoleExact404Count} consoleExact409ConflictCount=${consoleExact409ConflictCount} consoleFailedResource409OtherCount=${consoleFailedResource409OtherCount} consoleVueCapturedErrorCount=${consoleVueCapturedErrorCount} consoleCategoryOtherCount=${consoleCategoryOtherCount} consoleCategoryOtherApiErrorCount=${consoleCategoryOtherApiErrorCount} consoleCategoryOtherErrorCount=${consoleCategoryOtherErrorCount} consoleCategoryOtherVueWarnCount=${consoleCategoryOtherVueWarnCount} consoleCategoryOtherUncaughtCount=${consoleCategoryOtherUncaughtCount} consoleCategoryOtherAccessFetchCount=${consoleCategoryOtherAccessFetchCount} consoleCategoryOtherFailedResourceCount=${consoleCategoryOtherFailedResourceCount} consoleCategoryOtherGenericErrorCount=${consoleCategoryOtherGenericErrorCount} consoleCategoryOtherNonErrorPrefixCount=${consoleCategoryOtherNonErrorPrefixCount} consoleCategoryOther409TokenCount=${consoleCategoryOther409TokenCount} consoleKnownLinkedCount=${consoleKnownLinkedCount} consoleOtherCount=${consoleOtherCount}`
  }
