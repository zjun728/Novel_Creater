...
The service must commit `running` plus `attempt_id/lease` before `generate()`, then open a new transaction to finalize. `asyncio.timeout(180)` is the total whole-call deadline; the 240-second lease is therefore at least 60 seconds longer than any in-process call. Any `TimeoutError` or `httpx.TransportError` after the attempt marker is conservatively `outcome_unknown`; an explicit HTTP error response or strict response/option parse rejection is `failed`; pre-attempt configuration rejection performs zero transport calls and marks a stable configuration failure. Tests freeze every classification. Before strict parsing or option persistence, scan the full returned content for the current connection's complete API key and base URL, including stripped and common encoded forms. After JSON decoding but before any strict option construction, iteratively scan every container string again with fixed depth/node limits so Unicode and mixed escapes cannot bypass the gate. A match is a fixed `invalid_response`. Raw response plaintext is never persisted or exposed: successful/strictly-invalid content persists only the exact received UTF-8 SHA-256; a protocol failure without definite bytes uses `provider_failed` with no fabricated hash. Provider credentials and base URL never enter `request_json` or `request_hash`. Any expired running attempt reconciles to unknown and is never replayed. Do not import `backend/routers/ai_proxy.py`.
