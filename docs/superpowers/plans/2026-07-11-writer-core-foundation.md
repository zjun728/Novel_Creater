...
Define immutable `ProjectionBundle(revision, current_state, memories, arcs, plot_threads, content_hash)`. Strictly validate and freeze each event, reject duplicate IDs and duplicate `(revision_number, event_order)` stream positions, then sort by `(revision_number, event_order, id)`. `ProjectionEvent` equality and hashing use its complete canonical identity and preserve JSON distinctions between booleans, integers, floats and nested variants. Ignore rejected events, place every confirmed event into memory, exclude claims from current state, and derive arc/plot views only by `field_path` prefix. Plot threads use `entity_id` or `__global__` as the outer natural key so the same field on different entities never overwrites. Compute `content_hash` from canonical sorted JSON of `{revision,currentState,memories,arcs,plotThreads}`. The public canonical serializer accepts only raw strict JSON; Canon-internal frozen tuples use a private serializer. `FrozenJsonObject` rejects non-string and duplicate keys, normalizes key order, and preserves the equality/hash contract.
