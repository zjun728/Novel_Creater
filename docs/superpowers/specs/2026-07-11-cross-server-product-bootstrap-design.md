...
The whitelist contains one source capability query plus exactly three inventory queries for `projects`, `creative_seeds`, and `provider_profiles`. All four queries transport each result as `HEX(JSON_OBJECT(...))`, producing one ASCII-only hex record per physical output line. The reader captures stdout/stderr as bytes, strictly validates each ASCII hex line, decodes the recovered bytes as UTF-8, then parses exactly one JSON object; invalid hex, UTF-8, or JSON is rejected generically without rendering raw client output. Project and seed filters contain no non-ASCII SQL literals: they use fixed UTF-8 hex values with byte-exact `BINARY title = 0x...` and `IN (...)` predicates, so selection is independent of the MySQL 5.7 client's negotiated charset and collation. It never queries legacy derived tables or task bindings.
