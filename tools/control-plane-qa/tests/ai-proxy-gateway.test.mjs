...
test('keeps control-plane checks inside the unified product test scripts', async () => {
  const packageUrl = new URL('../../../package.json', import.meta.url)
  const packageJson = JSON.parse(await readFile(packageUrl, 'utf8'))

  assert.equal(packageJson.scripts.test, 'node scripts/run-tests.mjs unit')
  assert.equal(packageJson.scripts['test:integration'], 'node scripts/run-tests.mjs integration')
  assert.equal(packageJson.scripts['test:browser'], 'node scripts/run-tests.mjs browser')
  const controlPlaneScripts = {
  ...
  for (const [name, command] of Object.entries(controlPlaneScripts)) {
