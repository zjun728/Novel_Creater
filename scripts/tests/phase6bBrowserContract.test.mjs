...
test('Phase 6B owns one disposable UI-only browser lifecycle and verifier', async () => {
  ...
  const phase6aFixture = source('backend/scripts/prepare_phase6a_browser_db.py')
  assert.match(fixture, /CorpusImportService/u)
  assert.match(fixture, /CorpusRepository/u)
  assert.match(fixture, /await prepare_finalized_project\(database, corpus_source_refs=\(corpus_ref,\)\)/u)
  assert.match(fixture, /WHERE name=%s/u)
  assert.match(fixture, /Phase6A local deny/u)
  assert.doesNotMatch(fixture, /backend\.tests|planning_fixture|LEGACY_CORPUS_HASH|\bPROVIDER\b/u)
  assert.match(phase6aFixture, /corpus_source_refs: tuple\[CorpusSourceRef, \.\.\.\] = \(\)/u)
  assert.match(phase6aFixture, /corpusSourceRefs=tuple\(corpus_source_refs\)/u)
